...
export function createLodGeometry(size: number, segments: number, lod: number): BufferGeometry {
  ...
  const gridToWorld = (gridX: number, gridY: number): [number, number] => {
    return [(gridX / segments) * size - halfSize, (gridY / segments) * size - halfSize];
  };
  ...
  const findVerticesAlongEdge = (startCol: number, startRow: number, endCol: number, endRow: number): number[] => {
